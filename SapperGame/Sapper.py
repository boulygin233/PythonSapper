...
def main():
    ...
    class Field:
        def __init__(self, x=8, y=8, n=10):
            self.field = [[0 for i in range(y)] for j in range(x)]
            self.status = [[0 for i in range(y)] for j in range(x)]
            self.count = x * y - n
            c = 0
            while c < n:
        ...
        def leftclick(self, x_, y_):
            ...
            if self.status[x_][y_] == 0:
                if 0 <= self.field[x_][y_] <= 8:
                    num(self.field[x_][y_], x_, y_, color[self.field[x_][y_]])
                    self.count -= 1
                else:
                    mine(x_, y_, W, H)
                    GM = False
